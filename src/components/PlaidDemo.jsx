...
/* =========================================
   SINGLES NETWORK — Spending-Based Matching
   ========================================= */
function DatingMatching() {
  ...
  if (step === 0) {
    return (
      <div className="space-y-6">
        ...
        <p className="text-gray-400 text-sm leading-relaxed">
          AmandasList Singles has two layers of verification: identity (to keep out bots and catfish)
          and optional lifestyle matching (to find people who actually live like you do).
        </p>
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-dark-700 border border-pink-500/20">
            ...
            <p className="text-gray-500 text-xs mb-2">Government ID + selfie match via Plaid Identity. Ensures every person on the platform is real and KYC compliant.</p>
            <p className="text-gold-400 text-xs font-mono">$1.80/user</p>
          </div>
          <div className="p-4 rounded-lg bg-dark-700 border border-pink-500/20">
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2 py-0.5 rounded-full bg-dark-600 text-gray-400 text-[10px] font-semibold">OPTIONAL</span>
              <p className="text-white text-sm font-semibold">Lifestyle Matching via Transactions</p>
            </div>
            <p className="text-gray-500 text-xs mb-2">Connect your bank to match with people who share your spending habits. We analyze merchant categories only \u2014 never dollar amounts or account balances.</p>
            <p className="text-gold-400 text-xs font-mono">$1/user/month \u00B7 2 years history \u00B7 refreshes 3-4x daily</p>
          </div>
        </div>
        <div className="p-4 rounded-lg bg-dark-700 border border-white/10">
          <p className="text-white text-sm font-semibold mb-3">What We Analyze (Categories Only)</p>
          <div className="flex flex-wrap gap-2">
            {['Dining & Restaurants', 'Fitness & Wellness', 'Travel & Airlines', 'Shopping & Fashion', 'Entertainment', 'Coffee & Cafes', 'Groceries', 'Subscription Services'].map((c) => (
              <span key={c} className="px-3 py-1 rounded-full bg-pink-500/10 text-pink-400 text-xs border border-pink-500/20">{c}</span>
            ))}
          </div>
          <p className="text-gray-600 text-xs mt-3">We NEVER see: dollar amounts, account numbers, balances, or specific transaction details. Only merchant names and categories.</p>
        </div>
        <PrivacyNote>{"🔒 Your spending data is consent-based and user-permissioned. You can disconnect at any time. Data is never sold to third parties. Transaction data refreshes 3-4x daily for up-to-date matching."}</PrivacyNote>
        <PlaidLinkButton onClick={() => setStep(1)} label="Verify Identity + Connect Bank" />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5">
      <h3 className="text-xl font-bold text-white">Your Lifestyle Matches</h3>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="px-3 py-1 rounded-full bg-pink-500/15 text-pink-400 text-xs border border-pink-500/20">&#10003; Lifestyle Connected</span>
      </div>
      <p className="text-gray-400 text-sm">Matched based on shared spending patterns and lifestyle</p>
      {[
        {
          name: 'Sarah K.',
          age: 31,
        ...
        },
        {
          ...
          topCategory: 'Travel & Premium Dining',
        },
        {
          name: 'Maya L.',
          age: 29,
          ...
          topCategory: 'Shopping & Self-Care',
        },
      ].map((m, i) => (
        <div key={i} className="p-5 rounded-xl bg-dark-700 border border-white/10 hover:border-pink-500/30 transition cursor-pointer">
          ...
          <p className="text-gray-600 text-[10px]">Top shared category: {m.topCategory}</p>
        </div>
      ))}
      <SecondaryButton onClick={() => setStep(0)}>{"\u2190 Back to Start"}</SecondaryButton>
    </div>
